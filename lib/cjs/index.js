...
class CacheFile {
	...
	async set(key, content, options = {
		...
		if ("object" == type) {
			this.#error(`Content of ${key} shouldn't be Object`);
		}
		...
		await fs.outputFile(path.resolve(this.#config.folder, filePath), content)
			.then(async () => {
				...
				await this.#updateMetadata(key, filePath, size, type, {
					...optionDefault,
					...options,
	 ...
	 * @param {number} size - Path to the file.
	 * @param {string} type - Path to the file.
	 * @param {OptionsMetadata} options - Cache options.
	 * @private
	 */
	async #updateMetadata(key, filePath, size, type, options) {
	 ...
	 * @returns {Promise<DataMetadata|null>}
	 * @private
	 */
